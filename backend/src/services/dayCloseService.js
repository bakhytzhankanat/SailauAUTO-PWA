...
export async function createSnapshot(payload, user) {
  ...
  if (kaspi_amount > service_income_total) {
    throw new Error('KaspiPay сомасы қызмет кірісінен аспауы керек');
  }
  if (cash_amount === null) {
    cash_amount = Math.max(service_income_total - kaspi_amount, 0);
  } else if (cash_amount < 0) {
    throw new Error('Қолма-қол сома теріс болмауы керек');
  }
  if (kaspi_amount + cash_amount > service_income_total) {
    throw new Error('KaspiPay мен қолма-қол қызмет кірісінен аспауы керек');
  }
  ...
  let charity_raw = net_before_charity * (settings.charity_percent / 100);
  let charity_rounded = charity_raw;
  if (settings.round_charity_to_nearest_1000) {
  ...
  const master_amount = N > 0 ? masters_pool_amount / N : 0;
  ...
  try {
    ...
    for (const masterUserId of present_master_user_ids) {
      await client.query(
        ...
        [dc.id, masterUserId, master_amount]
      );
    }
    ...
    return {
      day_close: mapRow(dc),
      masters: masters.map((m) => ({ id: m.id, master_user_id: m.master_user_id, master_name: m.master_name, amount: Number(m.amount) })),
...
export async function updateSnapshot(id, payload, user) {
  ...
  let cash_amount = payload.cash_amount != null && payload.cash_amount !== '' ? num(payload.cash_amount) : null;
  if (cash_amount === null) cash_amount = Math.max(service_income_total - kaspi_amount, 0);
  if (kaspi_amount > service_income_total) throw new Error('KaspiPay сомасы қызмет кірісінен аспауы керек');
  if (cash_amount < 0) throw new Error('Қолма-қол сома теріс болмауы керек');
  if (kaspi_amount + cash_amount > service_income_total) throw new Error('KaspiPay мен қолма-қол қызмет кірісінен аспауы керек');
  ...
  const net_before_charity =
    service_income_total - material_expense_total - opex_lunch - opex_transport - opex_rent - kaspi_tax_amount;
  let charity_raw = net_before_charity * (settings.charity_percent / 100);
  let charity_rounded = settings.round_charity_to_nearest_1000 ? Math.round(charity_raw / 1000) * 1000 : charity_raw;
  const distributable_after_charity = net_before_charity - charity_rounded;
  const manager_amount = distributable_after_charity * (settings.manager_percent / 100);
  const remainder_after_manager = distributable_after_charity - manager_amount;
  const masters_pool_amount = remainder_after_manager * (settings.masters_percent / 100);
